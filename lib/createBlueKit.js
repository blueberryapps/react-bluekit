...
function isExluded(filename, exclude) {
  ...
  return exclude.reduce(function (acc, item) {
...
function getAllFilesInDir(dir) {
  ...
  return [].concat.apply([], _fs2.default.readdirSync(resolvedDir).map(function (file) {
    ...
    if (_fs2.default.lstatSync(absolutePath).isDirectory()) {
      return getAllFilesInDir(dir, _path2.default.join(relativeDirectory, file), exclude);
    }

    var filePath = _path2.default.join('./' + relativeDirectory, file);
    if (!filePath.match(/\.(js|jsx|tsx)$/)) return null;
    if (filePath.match(/__tests?__/)) return null;
    if (isExluded(filePath, exclude)) return null;
    return filePath;
  }));
}
...
function createBlueKit(config) {
  ...
  function generate() {
    var files = config.paths.map(function (file) {
      return getAllFilesInDir(config.baseDir, file, config.exclude);
    });
